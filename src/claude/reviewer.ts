...
import type { AiReviewer, AiReviewRequest, AiReviewResult, AiDiscussionRequest } from '../types.js'
...
export class ClaudeReviewer implements AiReviewer {
  ...
  async review(request: AiReviewRequest): Promise<AiReviewResult> {
    ...
    log.info({ model: this.model, mrTitle: request.mrTitle }, 'Requesting review')

    const response = await this.client.messages.create({
      model: this.model, max_tokens: 4096, system: REVIEW_SYSTEM_PROMPT,
  ...
  async respondToDiscussion(request: AiDiscussionRequest): Promise<string> {
    const response = await this.client.messages.create({
      model: this.model, max_tokens: 1024, system: DISCUSSION_SYSTEM_PROMPT,
      messages: [{
        role: 'user',
        content: `Your original comment:\n${request.originalComment}\n\nDeveloper's reply:\n${request.developerReply}\n\nCurrent diff context:\n\`\`\`\n${request.diffContext}\n\`\`\``,
      }],
    })
    return response.content[0].type === 'text' ? response.content[0].text : "I couldn't process that reply. Could you clarify?"
  }

  private buildReviewPrompt(request: AiReviewRequest): string {
    const parts: string[] = []
    parts.push(`## Merge Request\n**Title:** ${request.mrTitle}`)
    if (request.mrDescription) parts.push(`**Description:** ${request.mrDescription}`)
    if (request.customRules.length > 0) parts.push(`## Custom Rules\n${request.customRules.map((r) => `- ${r}`).join('\n')}`)
    if (request.focusAreas.length > 0) parts.push(`## Focus Areas\n${request.focusAreas.join(', ')}`)
    parts.push(`## Tone\n${request.tone}`)
  ...
  private parseResponse(text: string): AiReviewResult {
    ...
    try {
      ...
      return { summary: parsed.summary ?? 'Review complete.', findings: Array.isArray(parsed.findings) ? parsed.findings : [] }
    } catch {
      log.warn({ text: text.slice(0, 200) }, 'Failed to parse response as JSON')
